...
The Go server exposes seven tools over
[Streamable HTTP MCP](https://modelcontextprotocol.io/specification/2025-03-26/basic/transports#streamable-http):

| Tool | Direction | Platform | What it does |
|---|---|---|---|
| `log_reading_session` | write | PG + CH + Claude | Records a session; awards XP, updates streak transactionally, unlocks badges, generates a comprehension question |
| `get_student_progress` | read | PG | XP, level, streak, earned badges, gap to next badge, recent reads |
| `get_book_list` | read | PG | Catalogue filtered by genre or title fragment |
| `get_class_dashboard` | read | PG + CH | Students ranked by who most needs attention (cross-DB merge) |
| `recommend_book` | read | PG + Claude | Claude Opus 5 suggestion tailored to the student's reading history |
| `get_suspicious_sessions` | read | CH | ClickHouse analytics: rate anomalies + burst logging over the event stream |
| `answer_comprehension_question` | write | PG + Claude | Evaluates a student's free-text answer; stores result for teacher review |
...
ReadQuest addresses reward-hacking at three levels, each showcasing a different platform:

**1. Hard cap at write time (Go server)**
Sessions where `pages ÷ minutes > 5` are rejected before being stored.
A child reading 10 pages/minute sustained is physiologically implausible.

**2. Soft flag via ClickHouse analytics (`get_suspicious_sessions`)**
Two patterns flagged from the event stream — no Postgres involved:
- Rate anomaly: `pages/minutes > 2.0` (reviewable, not blocked)
- Burst logging: more than 2 sessions on the same calendar day

A teacher asks *"are any sessions suspicious?"* and ClickHouse answers from
the full 30-day history in milliseconds. This is the ClickHouse pitch: real
analytical SQL (`quantile`, `countIf`, `toFloat64`) over an append-only event
stream — not a slow Postgres `GROUP BY`.

**3. Comprehension check via Claude (`answer_comprehension_question`)**
After every session with a known-genre book, Claude generates one plot-specific
question (e.g. *"What was the first word Charlotte spun in her web?"*). The
student answers in the next chat turn; Claude evaluates the free text — warmly,
without a rubric — and the result is stored in Postgres for teacher review.

This is the multi-turn conversational flow that showcases **LibreChat** as an
interface rather than a single-shot tool caller, and **Claude** doing something
SQL genuinely cannot: understanding a book's content and judging a child's
expression of it without reducing the answer to a score.

XP is awarded regardless of the answer — the check is friction, not a gate.
...
session_questions (session_id, student_id, book_title, question, answer, evaluation, answered_at)
```

`session_questions` stores the comprehension question generated by Claude after
each session, the student's free-text answer, and Claude's evaluation. It gives
teachers a readable audit trail ("what Diego said about Hatchet") rather than a
binary pass/fail.
...
# MCP protocol smoke test (18 assertions, all 7 tools)
...
go run ./cmd/readquest-cli <command>
  ...
  suspicious [class]                        sessions flagged as suspicious (ClickHouse analytics)
...
|---|---|
| Identify struggling readers | `get_class_dashboard`: cross-DB merge ranks by days-silent and reading velocity; `get_suspicious_sessions`: ClickHouse analytics flags rate anomalies and burst logging |
| Help students find books | `get_book_list` + `recommend_book`: Claude Opus 5 personalised suggestions |
| **ClickHouse** | Event stream; at-risk aggregations (`countIf`, `sumIf`, `toFloat64`); suspicious session analytics over 30-day history; ClickHouse Cloud dashboards; native MCP for teacher ad-hoc queries |
| **PostgreSQL** | Source of truth for all state; transactional XP + streak + badge awards; comprehension question/answer history for teacher review |
| **LibreChat** | ClickHouse Agent is the only UI — students and teachers interact entirely through chat; comprehension check demonstrates a genuine multi-turn conversational flow |
| **AI/LLM (Claude)** | Book recommendations (tailored to reading history + inferred age band); comprehension question generation (plot-specific, from title + genre alone); comprehension answer evaluation (free text, no rubric, warm feedback) |
